  ...
  (d/function
   '{:lang :clojure
     ...
     :code (let [{game-id :game/id
             ...
             ;; 2. Adds a stat entity
             ;; 3. Adds player to the game
             ;; 4. Link player and stats entities to each other
             ;; 5. If the game is now full, add the :pending closed transaction
             (let [player-tmpid (d/tempid :db.part/user)
                   ...
                   player-trx {:db/id player-tmpid
                               ...
                               :player/color color}
                   stats-trx (merge {:db/id stats-tmpid} initial-stats)
                   join-trx {:db/id game-eid
                             :game/players player-tmpid}
                   link-stats-to-player-trx {:db/id player-tmpid
                                             :player/stats stats-tmpid}
                   link-player-to-stats-trx {:db/id stats-tmpid
               ...
               (cond-> [player-trx
                        stats-trx
                        join-trx
                        link-player-to-stats-trx
                        link-stats-to-player-trx]
