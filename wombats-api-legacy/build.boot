...
(require '[wombats-api.config.game :as game]
         ...
         '[wombats-api.services.github :refer [decode-bot]]
         '[wombats-api.test.game.test-game :refer [o b a f p]]
...
(deftask sim
  ...
  (let [{:keys [code ratelimit-message]} (get-bot-code-simulator username repo token)
    ...
    (when code
      (loop [{:keys [clean-arena messages players frames] :as game-state} initial-game-state
        ...
        (println "Mini Maps Before Resolution")

        (let [previous-frame (first frames)]
          (doall (map print-mini-map (into [] (:mini-maps previous-frame)))))
